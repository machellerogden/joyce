...
const typeMap = {
    ...
    str: {
        ...
        cast: (v, context, unquote) => unquote
    ...
    eval: {
        test: v => false,
        cast: (v, context) => Value([ "'", '"' ].includes(v.charAt(0)) && [ "'", '"' ].includes(v.charAt(v.length -1))
            ? v.slice(1, -1)
            : v, context)
    }
...
function Arg(arg, context) {
    ...
    if (typeMatch != null) {
        ...
        if (![ 'ref', 'eval' ].includes(type) && !argType.test(raw)) throw new Error(`invalid value \`${raw}\` for arg type \`${type}\` in arg: ${arg}`);
