...
const infer = v => {
    if (typeMap.String.test(v)) return typeMap.String.cast(v);
    if (typeMap.Number.test(v)) return typeMap.Number.cast(v);
    if (typeMap.Boolean.test(v)) return typeMap.Boolean.cast(v);
    if (typeMap.Object.test(v)) return typeMap.Object.cast(v);
    if (typeMap.Array.test(v)) return typeMap.Array.cast(v);
    return v;
}
