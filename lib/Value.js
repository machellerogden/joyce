...
/**
 * We're counting on ES6 spec for object traveral order to be upheld by the compiler.
 * Type inference is an ordered cascade.
 */
const types = Object.keys(typeMap);

const infer = (v, context) =>
    or(types.reduce((acc, type, i, arr) =>
        acc == null
            ? typeMap[type].test(v)
                ? typeMap[type].cast(v, context)
                : acc
            : acc,
        null), v);
...
function AstFromEL(str, context) {
    ...
    return compile(...rawArgs.map(a => Arg(a, context)));
}
